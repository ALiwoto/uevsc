...
export class WorkspaceIndexer implements vscode.Disposable {
    ...
    async rebuild(): Promise<void> {
        ...
        const exclude = configuration.get<string>(
            "exclude",
            "**/{Binaries,Build,DerivedDataCache,Intermediate,Saved,.git,node_modules}/**",
        );
    ...
    async indexUri(uri: vscode.Uri, document?: vscode.TextDocument): Promise<void> {
        ...
        try {
            ...
            let source: string;
            const openDocument =
                document ?? vscode.workspace.textDocuments.find((item) => item.uri.toString() === uri.toString());
            if (openDocument) {
            ...
            this.trace(
                `Indexed ${uri.fsPath}: ${parsed.symbols.length} symbols, ${parsed.parseErrors} errors, ${parsed.elapsedMs.toFixed(1)} ms`,
            );
        } catch (error) {
