...
export class SymbolIndex {
  ...
  resolve(context: ResolutionContext): ResolutionResult {
    ...
    const receiverType = qualifier ?? this.resolveReceiverType(
      context.uri,
      context.position,
      receiverName,
      receiverCall,
    ...
    let eligible = candidates.filter(
      (symbol) => !symbol.isLocal
        || (symbol.uri === context.uri && Boolean(symbol.visibilityRange) && contains(symbol.visibilityRange!, context.position)),
    );
    if (receiverType) {
  ...
  private resolveReceiverType(
    ...
    if (receiverCall) {
      ...
      const best = functions
        .filter((symbol) => !symbol.isLocal && (!containingType || !symbol.container || typeMatches(symbol.container, containingType)))
    ...
    const variable = file.symbols
      .filter((symbol) => symbol.name === receiverName
        && isBeforeOrAt(symbol.selectionRange.start, position)
        && (!symbol.isLocal || (Boolean(symbol.visibilityRange) && contains(symbol.visibilityRange!, position))))
  ...
  private findContainingTypeName(uri: string, position: SourcePosition): string | undefined {
    ...
    const enclosing = file.symbols
      .filter((symbol) => (symbol.kind === "class" || symbol.kind === "struct" || symbol.kind === "union") && contains(symbol.range, position))
...
function scoreSymbol(
  ...
  if (symbol.kind === "method" || symbol.kind === "function" || symbol.kind === "constructor" || symbol.kind === "destructor") score += 4;
  return score;
}
...
function compareSymbol(left: CppSymbol, right: CppSymbol): number {
  return left.qualifiedName.localeCompare(right.qualifiedName) || left.uri.localeCompare(right.uri) || comparePosition(left.selectionRange.start, right.selectionRange.start);
}
