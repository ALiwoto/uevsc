...
const CONTAINER_TYPES = new Set(["class_specifier", "struct_specifier", "union_specifier", "enum_specifier", "namespace_definition"]);
const DECLARATION_TYPES = new Set(["declaration", "field_declaration", "parameter_declaration", "optional_parameter_declaration"]);
...
export class CppParser {
  ...
  static async create(runtimeWasmPath: string, cppWasmPath: string): Promise<CppParser> {
    CppParser.runtimeInitialization ??= Parser.init({
      locateFile: () => runtimeWasmPath,
    });
    await CppParser.runtimeInitialization;

    const language = await Language.load(cppWasmPath);
  ...
  parse(uri: string, source: string): ParsedFile {
    ...
    if (!tree) {
      return { uri, symbols: [], parseErrors: 1, bytes: Buffer.byteLength(source), elapsedMs: performance.now() - started };
    }
  ...
  private extractContainer(
    ...
    const nameNode = node.childForFieldName("name") ?? findFirstDescendant(node, new Set(["type_identifier", "namespace_identifier"]));
    if (!nameNode) {
      return undefined;
    }
  ...
  private extractEnumerator(node: SyntaxNode, scope: readonly ScopeEntry[], uri: string, lines: readonly string[], output: CppSymbol[]): void {
  ...
  private extractAlias(node: SyntaxNode, scope: readonly ScopeEntry[], uri: string, lines: readonly string[], output: CppSymbol[]): void {
    const nameNode = node.childForFieldName("name") ?? findLastDescendant(node, new Set(["type_identifier", "identifier"]));
    if (!nameNode) {
      return;
    }
    const name = finalQualifiedPart(nameNode.text);
  ...
  private extractMacro(node: SyntaxNode, scope: readonly ScopeEntry[], uri: string, lines: readonly string[], output: CppSymbol[]): void {
...
function nearestTypeScope(scope: readonly ScopeEntry[]): ScopeEntry | undefined {
  return [...scope].reverse().find((entry) => entry.kind === "class" || entry.kind === "struct" || entry.kind === "union");
}
...
function isControlKeyword(name: string): boolean {
  return new Set(["if", "for", "while", "switch", "catch", "return", "sizeof", "alignof", "decltype", "static_assert"]).has(name);
}
...
function returnTypeFromSignature(signature: string, rawName: string): string | undefined {
  ...
  const prefix = signature.slice(0, nameIndex)
...
function maskUnrealAnnotations(source: string): string {
  ...
  const macroPattern = /\b(?:UCLASS|USTRUCT|UENUM|UINTERFACE|UFUNCTION|UPROPERTY|UMETA|GENERATED_BODY|GENERATED_UCLASS_BODY|GENERATED_USTRUCT_BODY|GENERATED_IINTERFACE_BODY|GENERATED_UINTERFACE_BODY)\b/g;
